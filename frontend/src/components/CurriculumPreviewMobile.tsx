...
const CurriculumPreviewMobile = () => {
  ...
  return (
    <div className="relative min-h-screen bg-gradient-to-br from-purple-900 via-violet-900 to-indigo-900 py-8 sm:py-12 lg:py-20 overflow-hidden lg:hidden">
      {/* Animated Background */}
      <div className="absolute inset-0">
        {/* Gradient Orbs - Reduced on mobile */}
        <motion.div
          className="absolute top-1/3 left-1/6 w-32 h-32 sm:w-48 sm:h-48 lg:w-64 lg:h-64 bg-purple-500/10 rounded-full blur-3xl"
          animate={{
            scale: [1, 1.3, 1],
            opacity: [0.2, 0.4, 0.2],
          }}
      ...
      <div className="relative z-30 max-w-7xl mx-auto px-4 sm:px-6">
        {/* Section Header */}
        <motion.div
        ...
        >
          <motion.div
            className="inline-flex items-center justify-center w-12 h-12 sm:w-16 sm:h-16 bg-gradient-to-r from-purple-500 to-violet-500 rounded-xl mb-4 sm:mb-6 shadow-2xl shadow-purple-500/25"
          ...
          <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold bg-gradient-to-r from-purple-200 via-violet-200 to-purple-300 bg-clip-text text-transparent mb-3 sm:mb-4">
            Cosmic Curriculum
          </h2>
          <p className="text-base sm:text-lg lg:text-xl text-purple-200/80 max-w-3xl mx-auto px-4">
        ...
        <div className="relative">
          ...
          <div className="relative">
              ...
              <div className="relative h-auto min-h-[500px]">
                <AnimatePresence mode="wait">
                  <motion.div
                    ...
                    className="w-full"
                  >
                    <div className={`relative bg-gradient-to-br ${currentTrackData.bgGradient} rounded-2xl sm:rounded-3xl border border-purple-300/20 backdrop-blur-sm overflow-hidden shadow-2xl`}>
                      {/* Card Glow Effect */}
                      ...
                      <div className="p-6 sm:p-8">
                        ...
                        <div className="mb-6 pr-16 sm:pr-20">
                          ...
                          <motion.p 
                            className="text-sm sm:text-base text-purple-200/90 leading-relaxed"
                            initial={{ opacity: 0, y: 20 }}
                            animate={{ opacity: 1, y: 0 }}
                        ...
                        >
                          <div className="text-center">
                            <div className="flex items-center justify-center mb-1 sm:mb-2">
                              ...
                              <span className="text-lg sm:text-xl font-bold text-white">{currentTrackData.level.split(' ')[0]}</span>
                            </div>
                            <span className="text-purple-300/70 text-xs sm:text-sm">Difficulty</span>
                          </div>
                          <div className="text-center">
                            <div className="flex items-center justify-center mb-1 sm:mb-2">
                              <Clock className="w-4 h-4 text-purple-400 mr-1" />
                              <span className="text-lg sm:text-xl font-bold text-white">{currentTrackData.duration}</span>
                            </div>
                            <span className="text-purple-300/70 text-xs sm:text-sm">Duration</span>
                          </div>
                          <div className="text-center">
                            <div className="flex items-center justify-center mb-1 sm:mb-2">
                              <Users className="w-4 h-4 text-blue-400 mr-1" />
                              <span className="text-lg sm:text-xl font-bold text-white">{currentTrackData.students}</span>
                            </div>
                            <span className="text-purple-300/70 text-xs sm:text-sm">Students</span>
                          </div>
                          <div className="text-center">
                            <div className="flex items-center justify-center mb-1 sm:mb-2">
                              <Trophy className="w-4 h-4 text-orange-400 mr-1" />
                              <span className="text-lg sm:text-xl font-bold text-white">{currentTrackData.projects}</span>
                            </div>
                            <span className="text-purple-300/70 text-xs sm:text-sm">Projects</span>
                          </div>
                        </motion.div>
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
                            {currentTrackData.topics.map((topic, index) => (
                              <motion.div
                                ...
                                className="p-2.5 sm:p-3 bg-white/10 backdrop-blur-sm rounded-lg border border-purple-300/20 hover:border-purple-300/40 transition-all duration-300"
                                whileHover={{ scale: 1.02, y: -1 }}
                              >
                                <span className="text-purple-100 font-medium text-sm sm:text-base">{topic}</span>
                              </motion.div>
                            ))}
                          </div>
                        </motion.div>
                        ...
                        <motion.button
                          ...
                          whileTap={{ scale: 0.98 }}
                          className={`w-full sm:w-fit px-6 sm:px-8 py-3 sm:py-4 bg-gradient-to-r ${currentTrackData.color} rounded-xl font-semibold text-white text-base sm:text-lg shadow-xl overflow-hidden group`}
                        >
                ...
                <div className="flex justify-between items-center mt-6">
                  <motion.button
                    onClick={prevTrack}
                    className="p-3 bg-purple-600/20 backdrop-blur-sm rounded-full border border-purple-300/20 hover:bg-purple-600/30 transition-all duration-300"
            ...
            <div className="hidden md:flex items-center h-[600px]">
              {/* Previous Button */}
              <motion.button
                onClick={prevTrack}
          ...
          {/* Track Preview Strip */}
          <motion.div 
            className="flex justify-center mt-8 sm:mt-12 space-x-2 sm:space-x-4 overflow-x-auto pb-4 px-4 sm:px-0"
          ...
          >
            {tracks.map((track, index) => (
              <motion.button
                key={track.id}
                onClick={() => selectTrack(index)}
                className={`flex-shrink-0 p-3 sm:p-4 rounded-lg sm:rounded-xl border transition-all duration-300 ${
                  index === currentTrack
              ...
              >
                <div className="flex items-center space-x-2 sm:space-x-3">
                  <div className={`p-1.5 sm:p-2 bg-gradient-to-r ${track.color} rounded-md sm:rounded-lg`}>
                    <track.icon className="w-3 h-3 sm:w-4 sm:h-4 text-white" />
                  </div>
                  <div className="text-left">
                    <div className="text-xs sm:text-sm font-semibold text-white">{track.title}</div>
      ...
      {/* Bottom Glow */}
      <div className="absolute bottom-0 left-0 right-0 h-16 sm:h-32 bg-gradient-to-t from-purple-900/50 to-transparent"></div>
    </div>
  );
};

export default CurriculumPreviewMobile;